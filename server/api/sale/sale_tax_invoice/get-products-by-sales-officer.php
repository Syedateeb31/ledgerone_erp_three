...
if (!$sales_officer_id) {
    ...
    echo json_encode(['success' => false, 'message' => 'Sales Officer ID is required']);
    exit;
}

try {
    // First, get all suppliers where salesman_id = sales_officer_id
    ...
    // Now get all products from these suppliers
    $stmt = $pdo->prepare("
        SELECT 
        ...
        AND p.vendor_id IN ($placeholders)
        ORDER BY p.name
    ");
