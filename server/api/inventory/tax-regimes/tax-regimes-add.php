...
try {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO tax_regimes (
            tenant_id, regime_name, regime_code, country_id, tax_authority, tax_base, legal_reference, finance_act_year,
            formula_template, formula_steps, formula_variables, applies_at_stage, application_level, is_tax_inclusive, is_single_stage,
            downstream_exempt, is_adjustable, is_refundable, is_final_tax, description,
    ...
    $stmt->execute([
        ...
        $formula_template, $formula_steps, $formula_variables, $applies_at_stage, $application_level, $is_tax_inclusive, $is_single_stage,
        $downstream_exempt, $is_adjustable, $is_refundable, $is_final_tax, $description,
        $is_active, $effective_from, $effective_to, $user_id
    ]);
    
    $regime_id = $pdo->lastInsertId();
    
    echo json_encode([
        'success' => true,
        'message' => 'Tax regime added successfully',
        'regime_id' => $regime_id
    ]);
