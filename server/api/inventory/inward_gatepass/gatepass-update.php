...
try {
    ...
    $stmt = $pdo->prepare("SELECT id FROM inward_gatepass WHERE id = ? AND tenant_id = ?");
    ...
    $stmt = $pdo->prepare("DELETE FROM inward_gatepass_items WHERE gatepass_id = ? AND tenant_id = ?");
    $stmt->execute([$input['id'], $tenant_id]);
