...
try {
    ...
    switch ($action) {
        case 'position':
            ...
            $cost_calculation = "AVG(sl.unit_cost)";
            if ($valuation_method === 'FIFO') {
                $cost_calculation = "(SELECT unit_cost FROM stock_ledger WHERE tenant_id = sl.tenant_id AND product_id = sl.product_id AND branch_id = sl.branch_id AND account_id = sl.account_id AND qty_in > 0 ORDER BY transaction_date ASC, id ASC LIMIT 1)";
            } elseif ($valuation_method === 'LIFO') {
                $cost_calculation = "(SELECT unit_cost FROM stock_ledger WHERE tenant_id = sl.tenant_id AND product_id = sl.product_id AND branch_id = sl.branch_id AND account_id = sl.account_id AND qty_in > 0 ORDER BY transaction_date DESC, id DESC LIMIT 1)";
            }
            ...
            $sql = "SELECT 
                        ...
                        COALESCE(SUM(CASE WHEN sl.transaction_date < ? THEN sl.qty_in ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN sl.transaction_date < ? THEN sl.qty_out ELSE 0 END), 0) as opening_balance,
                        ...
                        (COALESCE(SUM(CASE WHEN sl.transaction_date < ? THEN sl.qty_in ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN sl.transaction_date < ? THEN sl.qty_out ELSE 0 END), 0)) + (COALESCE(SUM(CASE WHEN sl.transaction_date >= ? AND sl.transaction_date <= ? THEN sl.qty_in ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN sl.transaction_date >= ? AND sl.transaction_date <= ? THEN sl.qty_out ELSE 0 END), 0)) as current_stock,
                        {$cost_calculation} as unit_cost,
                        ((COALESCE(SUM(CASE WHEN sl.transaction_date < ? THEN sl.qty_in ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN sl.transaction_date < ? THEN sl.qty_out ELSE 0 END), 0)) + (COALESCE(SUM(CASE WHEN sl.transaction_date >= ? AND sl.transaction_date <= ? THEN sl.qty_in ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN sl.transaction_date >= ? AND sl.transaction_date <= ? THEN sl.qty_out ELSE 0 END), 0))) * {$cost_calculation} as stock_value,
                        p.min_stock_level,
                        p.max_stock_level
            ...
            $params = [$from_date, $from_date, $from_date, $to_date, $from_date, $to_date, $from_date, $from_date, $from_date, $to_date, $from_date, $to_date, $from_date, $from_date, $from_date, $to_date, $from_date, $to_date, $tenant_id];
            ...
            $sql .= " GROUP BY sl.product_id, sl.branch_id, sl.account_id, u.uom_name ORDER BY COALESCE(p.parent_product_id, p.id), p.parent_product_id IS NULL DESC, p.name";
            ...
            $stats_sql = "SELECT 
                        ...
                        FROM (
                            SELECT 
                                ...
                                p.max_stock_level,
                                CASE 
                                    WHEN (COALESCE(SUM(sl.qty_in), 0) - COALESCE(SUM(sl.qty_out), 0)) <= 0 THEN 'out-of-stock'
                                    WHEN (COALESCE(SUM(sl.qty_in), 0) - COALESCE(SUM(sl.qty_out), 0)) < COALESCE(p.min_stock_level, 0) THEN 'low-stock'
                                    WHEN p.max_stock_level > 0 AND (COALESCE(SUM(sl.qty_in), 0) - COALESCE(SUM(sl.qty_out), 0)) > p.max_stock_level THEN 'overstock'
                                    ELSE 'in-stock'
                                END as status
                            FROM stock_ledger sl
                            JOIN products p ON sl.product_id = p.id
