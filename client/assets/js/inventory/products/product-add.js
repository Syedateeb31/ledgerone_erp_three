        ...
        if (data.success) {
            ...
            setTimeout(() => {
                ...
                if (product.tax_regime_id) {
                    document.getElementById('taxRegime').value = product.tax_regime_id;
                }
...
document.getElementById('taxRegime').addEventListener('change', function() {
    const warning = document.getElementById('eighthScheduleWarning');
    if (this.value === '16') {
...
// HS Code input handler for Eighth Schedule
let hsCodeDebounce;
document.getElementById('hsCode').addEventListener('input', function() {
    const hsCode = this.value.trim();
    if (document.getElementById('taxRegime').value === '16' && hsCode) {
        document.getElementById('displayHsCode').textContent = hsCode;
        
        clearTimeout(hsCodeDebounce);
        hsCodeDebounce = setTimeout(() => {
            fetch(`../../../../server/api/inventory/products/hs-code-rate.php?hs_code=${encodeURIComponent(hsCode)}`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    document.getElementById('displayTaxRate').textContent = parseFloat(data.rate).toFixed(2);
                    document.getElementById('displayLegalRef').textContent = data.legal_reference;
                } else {
                    document.getElementById('displayTaxRate').textContent = '18.00';
                    document.getElementById('displayLegalRef').textContent = 'Default rate (HS Code not found)';
                }
            })
            .catch(error => {
                console.error('Error fetching HS code rate:', error);
                document.getElementById('displayTaxRate').textContent = '18.00';
                document.getElementById('displayLegalRef').textContent = 'Error fetching rate';
            });
        }, 500);
    } else if (document.getElementById('taxRegime').value === '16') {
        document.getElementById('displayHsCode').textContent = '____';
        document.getElementById('displayTaxRate').textContent = '__';
        document.getElementById('displayLegalRef').textContent = '___________________';
    }
});
