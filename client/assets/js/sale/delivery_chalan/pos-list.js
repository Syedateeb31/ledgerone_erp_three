...
function initializeListPage(permissions) {
    ...
    function updateSelectionUI() {
        ...
        if (printSelectedBtn) printSelectedBtn.style.display = count > 0 ? 'inline-flex' : 'none';
    ...
    const _selectAll = document.getElementById('selectAllCheckbox'); if (_selectAll) _selectAll.addEventListener('change', function () {
    ...
    const _printBtn = document.getElementById('printSelectedBtn'); if (_printBtn) _printBtn.addEventListener('click', function () {
    ...
    const saleOfficerFilter = { value: '' };
    const supplierManFilter = { value: '' };
    ...
    async function loadInvoices(page = 1) {
        try {
            ...
            const response = await fetch(`../../../../server/api/sale/delivery_chalan/pos-list.php?${params.toString()}`);
    ...
    function populateTable(data) {
        ...
        data.forEach(invoice => {
            ...
            // Serial number cell
            row.insertCell(0).textContent = (currentPage - 1) * 10 + (invoices.indexOf(invoice) + 1);
            ...
            // Status badge
            const statusBadge = `<span style="padding:3px 8px;border-radius:4px;font-size:12px;background:${invoice.status === 'Posted' ? '#d4edda' : '#fff3cd'};color:${invoice.status === 'Posted' ? '#155724' : '#856404'}">${invoice.status}</span>`;
        ...
        document.getElementById('totalInvoiceCount').textContent = invoiceCount;
        document.getElementById('totalAmountSum').textContent = invoiceCount;
    }
    ...
    document.getElementById('resetFiltersBtn').addEventListener('click', function () {
        ...
        document.getElementById('customerFilterSearch').value = '';
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', async function() {
        if (deleteInvoiceId) {
            try {
                const response = await fetch(`../../../../server/api/sale/delivery_chalan/pos-delete.php?id=${deleteInvoiceId}`, {
                    method: 'DELETE'
                });
                const data = await response.json();
