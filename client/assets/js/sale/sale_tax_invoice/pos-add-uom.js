    ...
    if (product.uom_type === 'single' || product.uom_type === 'unit') {
        ...
        if (defaultUom) {
            ...
            if (defaultUom.unit_scope === 'per_product') {
                ...
                const response = await fetch(`../../../../server/api/sale/sale_tax_invoice/get-product-uom-conversions.php?product_id=${product.id}`);
...
async function loadUOMGroupUnits() {
    try {
        const response = await fetch('../../../../server/api/sale/sale_tax_invoice/get-uom-group-units.php');
        const data = await response.json();
        if (data.success) {
