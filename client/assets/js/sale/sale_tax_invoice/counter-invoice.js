...
async function loadDraftInvoices() {
    try {
        const response = await fetch('../../../../server/api/sale/sale_tax_invoice/get-drafts.php');
...
window.loadDraft = async function(draftId) {
    try {
        const response = await fetch(`../../../../server/api/sale/sale_tax_invoice/pos-edit.php?id=${draftId}`);
        const data = await response.json();
...
async function loadStockData() {
    ...
    try {
        const response = await fetch(`../../../../server/api/sale/sale_tax_invoice/get-product-stock.php?branch_id=${branchId}`);
        const data = await response.json();
        if (data.success) {
            updateStockDisplay();
        }
...
async function checkStockAndProceed(product) {
    ...
    try {
        const response = await fetch(`../../../../server/api/sale/sale_tax_invoice/get-product-stock.php?product_id=${product.id}&branch_id=${branchId}`);
        const data = await response.json();
    ...
    if (product.parent_product_id === null) {
        fetch(`../../../../server/api/sale/sale_tax_invoice/get-child-products.php?parent_id=${product.id}`)
...
async function checkAllItemsStock() {
    ...
    for (const input of qtyInputs) {
        ...
        try {
            const response = await fetch(`../../../../server/api/sale/sale_tax_invoice/get-product-stock.php?product_id=${productId}&branch_id=${branchId}`);
            const data = await response.json();
...
async function saveInvoice(status = 'Posted') {
    ...
    for (const item of currentInvoice.items) {
        if (item.productId) {
            try {
                const response = await fetch(`../../../../server/api/sale/sale_tax_invoice/get-product-stock.php?product_id=${item.productId}&branch_id=${currentInvoice.branch}`);
                const data = await response.json();
    ...
    try {
        const response = await fetch('../../../../server/api/sale/sale_tax_invoice/pos-add.php', {
            method: 'POST',
...
async function loadSupplierMen() {
    const response = await fetch('../../../../server/api/sale/sale_tax_invoice/get-supplier-men.php');
    const data = await response.json();
    if (data.success) {
