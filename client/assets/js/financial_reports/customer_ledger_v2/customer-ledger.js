document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadCompanies() {
        fetch('../../../../server/api/financial_reports/customer_ledger_v2/get-companies.php')
    ...
    async function loadCurrencies() {
        try {
            const response = await fetch('../../../../server/api/financial_reports/customer_ledger_v2/customer-ledger.php?type=currencies');
    ...
    async function loadCustomers() {
        try {
            const response = await fetch('../../../../server/api/financial_reports/customer_ledger_v2/customer-ledger.php?type=customers');
            const result = await response.json();
    ...
    async function loadDistributions() {
        try {
            const response = await fetch('../../../../server/api/financial_reports/customer_ledger_v2/customer-ledger.php?type=distributions');
            const result = await response.json();
    ...
    async function loadSubAccounts(customerId) {
        try {
            const response = await fetch(`../../../../server/api/financial_reports/customer_ledger_v2/customer-ledger.php?type=sub_accounts&customer_id=${customerId}`);
            const result = await response.json();
    ...
    async function loadLedgerData() {
        ...
        try {
            const response = await fetch(`../../../../server/api/financial_reports/customer_ledger_v2/customer-ledger.php?${params}`);
            const result = await response.json();
    ...
    function renderDetailedLedger(data, openingBalance) {
        ...
        let totalDebit = 0, totalCredit = 0, totalMarkup = 0;
        let rowIndex = 0;

        // Helper: format date as YYYY-MM-DD string
        function toDateStr(d) {
            if (!d || d === 'Opening Balance') return null;
            const dt = new Date(d);
        ...
        // Helper: daily markup = balance * (markupPercent / 100) / 30
        function calcDailyMarkup(balance) {
            if (!markupPercent || balance === 0) return 0;
            return Math.abs(balance) * (markupPercent / 100) / 30;
        }

        // Helper: render a transaction row (returns HTML string)
        function txnRowHTML(row, debit, credit, balance, pdcDisplay, hasItems, rIdx) {
            ...
            return `
                <tr class="ledger-row" data-row-id="${rIdx}">
                    <td>${row.date}</td>
                    <td>
                        ${hasItems ? `<button class="expand-btn" data-row-id="${rIdx}"><i class="fas fa-chevron-right"></i></button>` : ''}
                        ${row.description}
                    </td>
                    <td>${row.reference}</td>
        ...
        // Helper: render markup row for a date
        function markupRowHTML(dateStr, balance, markup) {
            const displayDate = dateStr ? new Date(dateStr).toLocaleDateString('en-GB') : '-';
            return `
                <tr style="background: rgba(255,193,7,0.06);">
                    <td>${displayDate}</td>
                    <td style="color: var(--warning); font-style: italic;">- Balance &amp; Markup Value</td>
                    <td>-</td>
                    <td></td>
                    <td></td>
                    <td class="${balance >= 0 ? 'balance-positive' : 'balance-negative'}">${currentCurrencySymbol}${Math.abs(balance).toFixed(2)} ${balance >= 0 ? 'Dr' : 'Cr'}</td>
                    <td style="color: var(--warning); font-weight: 600;">${markup.toFixed(2)}</td>
                </tr>
            `;
        }
        ...
        data.forEach((row) => {
            if (row.type === 'opening_balance') {
                ...
                `;
                lastBalance = balance;
                if (dateStr) lastTxnDate = dateStr;

                // Same-day markup row after opening balance
                if (markupPercent > 0 && dateStr && balance !== 0) {
                    const mu = calcDailyMarkup(balance);
                    totalMarkup += mu;
                    tbody.innerHTML += markupRowHTML(dateStr, balance, mu);
                }
                ...
                const currentDateStr = toDateStr(row.date);

                totalDebit += debit;
                totalCredit += credit;

                // Fill markup rows: from day AFTER lastTxnDate up to day BEFORE currentDateStr
                if (markupPercent > 0 && lastTxnDate && currentDateStr && lastBalance !== 0) {
                    const nextDay = new Date(lastTxnDate);
                    nextDay.setDate(nextDay.getDate() + 1);
                    const nextDayStr = nextDay.toISOString().split('T')[0];
                    const prevDay = new Date(currentDateStr);
                    prevDay.setDate(prevDay.getDate() - 1);
                    const prevDayStr = prevDay.toISOString().split('T')[0];
                    if (nextDayStr <= prevDayStr) {
                        getDaysBetween(nextDayStr, prevDayStr).forEach((dayStr) => {
                            const mu = calcDailyMarkup(lastBalance);
                            totalMarkup += mu;
                            tbody.innerHTML += markupRowHTML(dayStr, lastBalance, mu);
                        });
                    }
                }

                // Render the actual transaction row
                tbody.innerHTML += txnRowHTML(row, debit, credit, balance, pdcDisplay, hasItems, rowIndex);
                ...
                // Same-day markup row after transaction (using new balance)
                if (markupPercent > 0 && currentDateStr && balance !== 0) {
                    const mu = calcDailyMarkup(balance);
                    totalMarkup += mu;
                    tbody.innerHTML += markupRowHTML(currentDateStr, balance, mu);
                }

                lastBalance = balance;
                if (currentDateStr) lastTxnDate = currentDateStr;
                rowIndex++;
            }
        });

        // Fill markup rows from day AFTER last transaction date to toDate
        if (markupPercent > 0 && lastTxnDate && lastBalance !== 0) {
            const endDateStr = toDate.value || lastTxnDate;
            const nextDay = new Date(lastTxnDate);
            nextDay.setDate(nextDay.getDate() + 1);
            const nextDayStr = nextDay.toISOString().split('T')[0];
            if (nextDayStr <= endDateStr) {
                getDaysBetween(nextDayStr, endDateStr).forEach((dayStr) => {
                    const mu = calcDailyMarkup(lastBalance);
                    totalMarkup += mu;
                    tbody.innerHTML += markupRowHTML(dayStr, lastBalance, mu);
                });
        ...
        tbody.innerHTML += `
            <tr class="totals-row">
                ...
                <td><strong style="color: var(--warning);">${totalMarkup.toFixed(2)}</strong></td>
            </tr>
        `;
    ...
    document.getElementById('print-ledger').addEventListener('click', function(e) {
        ...
        const markupVal = document.getElementById('markup-percent').value;
        if (markupVal) {
            params.append('markup_percent', markupVal);
        }
