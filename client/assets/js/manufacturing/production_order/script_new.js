(function() {
    ...
    function renderMaterialsTable(materials) {
        ...
        materials.forEach(mat => {
            if (mat.units.length > maxUnits) maxUnits = mat.units.length;
        });
        ...
        materials.forEach(mat => {
            ...
            for (let i = 0; i < maxUnits; i++) {
                ...
                if (i < mat.units.length) {
                    ...
                    unitCell.innerHTML = `<div style="text-align: center; color: #9AA1AE;"><div style="font-weight: 600;">-</div></div>`;
            ...
            mat.units.forEach(unit => {
                const s = Number(unit.available_stock) || 0;
                const c = Number(unit.conversion_factor) || 1;
                stockTotal = (unit.is_base_unit == 1) ? s : (stockTotal + s * c);
            });
            stockCell.textContent = stockTotal.toFixed(2);
            tr.appendChild(stockCell);
            
            const statusCell = document.createElement('td');
            let reqTotal = 0;
            mat.units.forEach(unit => {
                const r = Number(unit.required_qty) || 0;
                const c = Number(unit.conversion_factor) || 1;
                reqTotal = (unit.is_base_unit == 1) ? r : (reqTotal + r * c);
            });
            
            const isAvailable = stockTotal >= reqTotal;
            statusCell.innerHTML = `<span class="badge ${isAvailable ? 'available' : 'short'}">${isAvailable ? 'Available' : 'Short'}</span>`;
