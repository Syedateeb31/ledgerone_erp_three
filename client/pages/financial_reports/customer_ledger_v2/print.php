...
$markup_percent = floatval($_GET['markup_percent'] ?? 0);
...
<body>
    ...
    <div class="header">
        ...
        <div class="report-title">Customer MarkUp Ledger Report (<?php echo ucfirst($ledger_type); ?>)</div>
    </div>
    ...
    <div class="report-info">
        ...
        <?php endif; ?>
        <?php if ($markup_percent > 0): ?>
        <div><strong>MarkUp (%):</strong> <?php echo $markup_percent; ?>%</div>
        <?php endif; ?>
    </div>
    ...
    <script>
        ...
        const markupPercent = <?php echo $markup_percent; ?>;
        const expandedRows = '<?php echo $expanded_rows; ?>'.split(',').filter(r => r);
        const toDateStr = '<?php echo $date_to; ?>';
        ...
        fetch(`../../../../server/api/financial_reports/customer_ledger_v2/customer-ledger.php?${params}`)
        ...
        async function renderDetailedReport(data, openingBalance) {
            ...
            <?php if ($customer_id): ?>
            try {
                const customerResponse = await fetch(`../../../../server/api/financial_reports/customer_ledger_v2/customer-ledger.php?type=customers`);
            ...
            // Get detailed data
            const detailedUrl = '../../../../server/api/financial_reports/customer_ledger_v2/customer-ledger.php?type=detailed&customer_id=<?php echo $customer_id; ?><?php if ($date_from): ?>&from_date=<?php echo $date_from; ?><?php endif; ?><?php if ($date_to): ?>&to_date=<?php echo $date_to; ?><?php endif; ?><?php if ($distribution_id): ?>&distribution_id=<?php echo $distribution_id; ?><?php endif; ?><?php if ($sub_account_id): ?>&sub_account_id=<?php echo $sub_account_id; ?><?php endif; ?><?php if ($company_id): ?>&company_id=<?php echo $company_id; ?><?php endif; ?>';
            ...
            function markupRowHTML(dateStr, balance, markup) {
                return `
                    <tr style="background: #fffbea;">
                        ...
                        <td style="color: #b8860b; font-style: italic;">- Balance &amp; Markup Value</td>
                        <td>-</td>
                        <td></td>
                        <td></td>
                        <td class="${balance >= 0 ? 'balance-positive' : 'balance-negative'}">${currencySymbol}${Math.abs(balance).toFixed(2)} ${balance >= 0 ? 'Dr' : 'Cr'}</td>
                        <td style="color: #b8860b; font-weight: 600;">${markup.toFixed(2)}</td>
                    </tr>
                `;
            }
            ...
            data.forEach(row => {
                if (row.type === 'opening_balance') {
                    ...
                    if (markupPercent > 0 && dateStr && balance !== 0) {
                        ...
                        totalMarkup += mu;
                        html += markupRowHTML(dateStr, balance, mu);
                    }
                    ...
                    const currentDateStr = toDateString(row.date);
                    ...
                    if (markupPercent > 0 && lastTxnDate && currentDateStr && lastBalance !== 0) {
                        ...
                        if (nextDayStr <= prevDayStr) {
                            getDaysBetween(nextDayStr, prevDayStr).forEach((dayStr) => {
                                ...
                                totalMarkup += mu;
                                html += markupRowHTML(dayStr, lastBalance, mu);
                            });
                        }
                    }
                    ...
                    if (markupPercent > 0 && currentDateStr && balance !== 0) {
                        const mu = calcDailyMarkup(balance);
                        totalMarkup += mu;
                        html += markupRowHTML(currentDateStr, balance, mu);
                    }
            ...
            // Fill remaining markup rows: from day AFTER last txn date to toDate
            if (markupPercent > 0 && lastTxnDate && lastBalance !== 0) {
                const endDate = toDateStr || lastTxnDate;
                const nextDay = new Date(lastTxnDate);
                nextDay.setDate(nextDay.getDate() + 1);
                const nextDayStr = nextDay.toISOString().split('T')[0];
                if (nextDayStr <= endDate) {
                    getDaysBetween(nextDayStr, endDate).forEach((dayStr) => {
                        const mu = calcDailyMarkup(lastBalance);
                        totalMarkup += mu;
                        html += markupRowHTML(dayStr, lastBalance, mu);
                    });
            ...
            html += `
                    <tr class="totals-row">
                        ...
                        <td><strong>${currencySymbol}${totalDebit.toFixed(2)}</strong></td>
                        <td><strong>${currencySymbol}${totalCredit.toFixed(2)}</strong></td>
                        <td><strong>${currencySymbol}${Math.abs(lastRunningBalance).toFixed(2)} ${lastRunningBalance >= 0 ? 'Dr' : 'Cr'}</strong></td>
                        <td><strong style="color: #b8860b;">${totalMarkup.toFixed(2)}</strong></td>
                    </tr>
                </tbody>
            </table>
            <p><strong>Amount in Words:</strong> ${numberToWords(Math.abs(lastRunningBalance))} ${lastRunningBalance >= 0 ? 'Debit' : 'Credit'} Only</p>
            `;
